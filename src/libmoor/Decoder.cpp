...
unsigned char* 
base64_decode(const char* data, int length, unsigned int* read) {
	assert(data);
	assert(length > 0);
	...
	b64 = BIO_new(BIO_f_base64());
	BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL);
	bmem = BIO_new_mem_buf(const_cast<char*>(data), length);
	bmem = BIO_push(b64, bmem);

	if (read) 
		*read = BIO_read(bmem, buffer, length);
	else 
		BIO_read(bmem, buffer, length);
             ...
             unsigned char* key, unsigned char* iv)
{
	...
	mdecrypt_generic(thread, *data, data_size);
